...
class adjust_bottle_idle(Base_Task):

    def setup_demo(self, **kwags):
        super()._init_task_env_(**kwags)
        # Optional knobs to stretch episode length for video/world-model data
        self.extra_idle_steps = kwags.get("extra_idle_steps", 0)
        self.hover_idle_steps = kwags.get("hover_idle_steps", 0)
        self.post_place_idle_steps = kwags.get("post_place_idle_steps", 0)
        self.sweep_offset = kwags.get("sweep_offset", 0.1)
        self.sweep_repeat = kwags.get("sweep_repeat", 9)
    ...
    def play_once(self):
        ...
        # Hold briefly to extend clip length before moving
        if self.extra_idle_steps:
            self.delay(self.extra_idle_steps)

        # Move the arm upward by 0.1 meters along z-axis
        self.move(self.move_by_displacement(arm_tag=arm_tag, z=0.1, move_axis="arm"))

        # Hover and perform gentle lateral sweeps to lengthen trajectory
        if self.hover_idle_steps:
            self.delay(self.hover_idle_steps)
        if self.sweep_offset and self.sweep_repeat:
            for _ in range(max(1, self.sweep_repeat)):
                self.move(self.move_by_displacement(arm_tag=arm_tag, x=self.sweep_offset, move_axis="arm"))
                self.move(self.move_by_displacement(arm_tag=arm_tag, x=-self.sweep_offset, move_axis="arm"))
        ...
        # Keep pose after placement to add tail frames
        if self.post_place_idle_steps:
            self.delay(self.post_place_idle_steps)
